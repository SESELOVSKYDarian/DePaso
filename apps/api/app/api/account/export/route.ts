...
export async function GET(request: NextRequest) {
  ...
  const body: AccountExportResponse = {
    ...
    savedRouteContexts: savedRouteContexts.map((r) => ({
      id: r.id,
      name: r.name,
    ...
    shoppingLists: shoppingLists.map((l) => ({
      ...
      items: l.items.map((i) => ({ productId: i.productId, quantity: i.quantity, note: i.note })),
    })),
    productPreferences: productPreferences.map((p) => ({
      productId: p.productId,
      type: p.type,
      preferredBrandId: p.preferredBrandId,
    })),
    storePreferences: storePreferences.map((p) => ({
      category: p.category,
      type: p.type,
